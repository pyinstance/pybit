            ...
            if urls:
                for url in urls:
                    if validators.url(url):
                        results.append(url)
                        # Commented out to suppress printing URLs
...
def genhtmlrep(urls, log_dir, source_file, pycdc_log):
    # Read the source code from the PyCDAS source file
    try:
    ...
    # Read the source code from the PyCDC log file
    try:
...
if __name__ == '__main__':
    ...
    if pycdas_log and pycdc_log:
        ...
        genhtmlrep(urls, os.path.dirname(pycdas_log), pycdas_log, pycdc_log)   
